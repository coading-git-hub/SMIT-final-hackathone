...
const StylesList = () => {
  ...
  useEffect(() => {
    const fetchStyles = async () => {
      setLoading(true);
      const listRes = await httpAction({ url: apis().listStyles });
      if (listRes?.status) {
        const items = listRes.styles || [];
        if (items.length < 6) {
          // Reseed  all six demo cards appear
          await httpAction({ url: apis().seedStyles });
          const afterSeed = await httpAction({ url: apis().listStyles });
          setStyles(afterSeed?.styles || []);
        } else {
          setStyles(items);
        }
      }
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 6 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' }, gap: 3, alignItems: 'stretch' }}>
        {!loading && (!styles || styles.length === 0) && (
